...
def get_platform():
  ...
  return "Mac_Arm" if arch == "arm64" else "Mac"
...
def get_current_rev():
  ...
  revision_file = Path(APP) / "Contents" / "revision"
  try:
...
def download_snapshot(tmp_dir, revision):
  ...
  zip_url = f"{SNAP_BASE}/{PLATFORM}/{revision}/chrome-mac.zip"
  zip_path = tmp_dir / "chrome-mac.zip"

  print(f"Downloading Chromium {revision} for {PLATFORM}")
  curl(zip_url, zip_path)
  unzip(zip_path, tmp_dir)
  return tmp_dir / "chrome-mac" / "Chromium.app"

def install_snapshot(src_app, revision):
  """Install Chromium snapshot, replacing existing installation."""
  # Kill any running Chromium processes
  run(["pkill", "-f", "Chromium"], check=False)
  ...
  shutil.copytree(src_app, app_path)

  # Store revision number for future comparisons
  revision_file = app_path / "Contents" / "revision"
...
def install_codecs():
  """Install libffmpeg.dylib for codec support."""
  lib_dir = Path(LIB_DIR)
  lib_dir.mkdir(parents=True, exist_ok=True)
  libffmpeg_path = lib_dir / "libffmpeg.dylib"

  # Backup existing if present
  if libffmpeg_path.exists() and not (lib_dir / "libffmpeg.dylib.backup").exists():
    shutil.copy2(libffmpeg_path, lib_dir / "libffmpeg.dylib.backup")

  # Determine source
  if os.getenv("LIBFFMPEG_PATH") and Path(os.getenv("LIBFFMPEG_PATH")).exists():
    shutil.copy2(os.getenv("LIBFFMPEG_PATH"), libffmpeg_path)
  elif os.getenv("LIBFFMPEG_URL"):
    with tempfile.NamedTemporaryFile() as tmp:
      curl(os.getenv("LIBFFMPEG_URL"), tmp.name)
      shutil.copy2(tmp.name, libffmpeg_path)
  else:
    # Auto-fetch from iteufel
    ffmpeg_ver = os.getenv("FFMPEG_AUTO_VER", "0.87.0")
    arch = "arm64" if PLATFORM == "Mac_Arm" else "x64"
    url = f"https://github.com/iteufel/nwjs-ffmpeg-prebuilt/releases/download/{ffmpeg_ver}/{ffmpeg_ver}-osx-{arch}.zip"
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
      ...
      # Find libffmpeg.dylib in extracted files
      for ffmpeg_file in Path(tmp_dir).rglob("libffmpeg.dylib"):
        shutil.copy2(ffmpeg_file, libffmpeg_path)
        break
      else:
        raise FileNotFoundError("libffmpeg.dylib not found in downloaded archive")

  # Remove codesign signature
  run(["codesign", "--remove-signature", str(libffmpeg_path)], check=False)

  # Verify installation
  try:
    run(["otool", "-L", str(libffmpeg_path)], capture_output=True)
    print("libffmpeg OK")
...
def install_widevine():
  ...
  widevine_dst = Path(LIB_DIR) / "WidevineCdm"

  # Determine source
  widevine_src_env = os.getenv("WIDEVINE_SRC")
  if widevine_src_env and Path(widevine_src_env).exists():
    if widevine_dst.exists():
      shutil.rmtree(widevine_dst)
    shutil.copytree(widevine_src_env, widevine_dst)
  else:
    chrome_widevine = Path("/Applications/Google Chrome.app/Contents/Frameworks/Google Chrome Framework.framework/Libraries/WidevineCdm")
    if chrome_widevine.exists():
      if widevine_dst.exists():
        shutil.rmtree(widevine_dst)
      shutil.copytree(chrome_widevine, widevine_dst)
    else:
      # Download Chrome DMG and extract Widevine
      with tempfile.TemporaryDirectory() as tmp_dir:
        tmp_path = Path(tmp_dir)
        dmg_path = tmp_path / "chrome.dmg"
        mount_point = tmp_path / "chrome_mount"

        # Download Chrome DMG
        arch_param = "arm64" if PLATFORM == "Mac_Arm" else "x64"
        chrome_url = f"https://dl.google.com/chrome/mac/universal/stable/GGRO/googlechrome.dmg"
        curl(chrome_url, dmg_path)

        # Mount DMG
        mount_point.mkdir()
        result = run(["hdiutil", "attach", "-nobrowse", "-readonly", "-mountpoint",
                     str(mount_point), str(dmg_path)], capture_output=True)

        try:
          mounted_chrome_widevine = mount_point / "Google Chrome.app/Contents/Frameworks/Google Chrome Framework.framework/Libraries/WidevineCdm"
          if mounted_chrome_widevine.exists():
            if widevine_dst.exists():
              shutil.rmtree(widevine_dst)
            shutil.copytree(mounted_chrome_widevine, widevine_dst)
          else:
            print("Warning: WidevineCdm not found in Chrome DMG")
            return
        finally:
          # Detach DMG
  ...
  # Remove quarantine attribute
  run(["xattr", "-dr", "com.apple.quarantine", str(widevine_dst)], check=False)
...
def main():
  ...
  need_install = not Path(APP).exists() or force_update or current_rev != latest_rev

  if need_install:
    # Download and install Chromium
    with tempfile.TemporaryDirectory() as tmp_dir:
      src_app = download_snapshot(tmp_dir, latest_rev)
      install_snapshot(src_app, latest_rev)
